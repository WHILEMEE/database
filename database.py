...
#每个容器池表放入20个数据
        ...
        temp1 = random.randint(4, 8)
        cur.execute('''
                    INSERT INTO ChildD_ip (current_ip_address,System_container,Language_container,Work_container,cpu_number,memory)
                    SELECT %s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM ChildD_ip WHERE current_ip_address = %s);
                ''', (ip_addresses[3], categorys[0],categorys[1],categorys[2], temp1, temp2, ip_addresses[3]))
        temp1 = random.randint(4, 8)
        cur.execute('''
                    INSERT INTO ChildE_ip (current_ip_address,System_container,Language_container,Work_container,cpu_number,memory)
                    SELECT %s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM ChildE_ip WHERE current_ip_address = %s);
                ''', (ip_addresses[4], categorys[0],categorys[1],categorys[2], temp1, temp2, ip_addresses[4]))
        temp1 = random.randint(4, 8)
        cur.execute('''
                    INSERT INTO ChildF_ip (current_ip_address,System_container,Language_container,Work_container,cpu_number,memory)
                    SELECT %s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM ChildF_ip WHERE current_ip_address = %s);
                ''', (ip_addresses[5], categorys[0],categorys[1],categorys[2], temp1, temp2, ip_addresses[5]))
        ...
        for i in range(0, 5):
            ...
            cur.execute('''
                    INSERT INTO Task (task_id,cpu_number,memory,status,System_container,Language_container,Work_container)
                    SELECT %s,%s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM Task WHERE task_id = %s);
                ''', (T_id, temp1, temp2, status,Is_needs[0],Is_needs[1],Is_needs[1], T_id))
        ...
        for i in range(5, 10):
            ...
            cur.execute('''
                    ...
                    SELECT %s,%s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM Task WHERE task_id = %s);
                ''', (T_id, temp1, temp2, status,Is_needs[0],Is_needs[0],Is_needs[1], T_id))
        ...
        for i in range(10, 15):
            ...
            cur.execute('''
                    ...
                    SELECT %s,%s,%s,%s,%s,%s,%s
                    WHERE NOT EXISTS (SELECT 1 FROM Task WHERE task_id = %s);
                ''', (T_id, temp1, temp2, status,Is_needs[0],Is_needs[1],Is_needs[0], T_id))

        for i in range(0, 15):
            # Task_System表
            T_id = f'Task{i + 1}'
            category_name = system_names[random.randint(0,4)]
            cur.execute('''
        ...
        for i in range(0, 5):
            ...
            T_id = f'Task{i + 6}'
            category_name = language_names[random.randint(0,4)]
            cur.execute('''
            ...
            T_id = f'Task{i + 11}'
            category_name = work_names[random.randint(0,4)]
            cur.execute('''
